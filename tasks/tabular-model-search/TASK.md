...
This task follows the repository-level experiment protocol (encoded in the
`autoresearch-experiment` agent) with these
...
deep-tuning), not by hand; see the experiment protocol. Candidate files under `runs/` are
...
(one record per run; see `.opencode/rules/ledger.md`). `ledger.py record-run`
