"""
Autoresearch pretraining candidate — multi-GPU (DDP) variant. Single-file.
...
Multi-GPU: `env.rank`/`env.world_size` identify this process's DDP rank
...
def _maybe_init_distributed(env):
    """Pin this rank's device and (world_size > 1) join the NCCL group.

    Returns (rank, world_size, ddp_active). With world_size == 1 this is just
    device selection and no process group is created, so single-GPU runs are
    byte-identical in behavior to the single-GPU task."""
    rank = int(getattr(env, "rank", 0))
    world_size = int(getattr(env, "world_size", 1))
    torch.cuda.set_device(rank)
    ddp_active = world_size > 1
    if ddp_active and not dist.is_initialized():
        dist.init_process_group(backend="nccl")
    return rank, world_size, ddp_active


def _unwrap_model(model):
    """Peel torch.compile / DDP wrappers down to the raw GPT module."""
    m = model
    while True:
...
class Trainer:
    ...
    def preflight(self):
        ...
        rank, world_size, ddp_active = _maybe_init_distributed(env)
        ...
        # The process group is left up on purpose: the fixed probe wrapper in
        # prepare.py max-reduces peak_vram_mb across ranks after this returns
        # (worker processes exit right after, which reclaims the group).
        return {
    ...
    def run(self):
        ...
        H100_BF16_PEAK_FLOPS = 989.5e12 * world_size  # aggregate over ranks
        ...
        total_batch_size = tokens_per_fwdbwd * grad_accum_steps * world_size
        ...
        while True:
            ...
            done = step > 10 and total_training_time >= budget
            if ddp_active:
                # Ranks' clocks can drift; all ranks must break on the same
                # step or the laggard hangs in the next gradient allreduce.
                done_t = torch.tensor([1.0 if done else 0.0], device=device)
        ...
        if ddp_active:
            # Report the worst-rank peak; symmetric ranks should be close.
            peak_t = torch.tensor([float(peak_vram_bytes)], dtype=torch.float64, device=device)
            dist.all_reduce(peak_t, op=dist.ReduceOp.MAX)
            peak_vram_bytes = peak_t.item()
