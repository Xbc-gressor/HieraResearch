...
由 opencode 驱动的自主 AutoML 实验框架。该框架使 Claude 能够迭代地提出候选解决方案、修改代码、调优超参数、评估结果，并仅保留改进——适用于多个研究任务。
...
- **实验协议（编码在 `autoresearch-experiment` 智能体定义中）**：主会话执行的规范实验协议。
- **`AGENTS.md`**：opencode 打开项目时首先读取的入口文档。
- **`.opencode/agents/`**：用于复杂多步骤任务的专用子智能体（想法生成、代码编写、合约提取、调优编排）。
- **`.opencode/skills/`**：可复用的内联方法论（当前：崩溃诊断）。
...
- **`tasks/tabular-blind/`**：跨三个匿名化真实数据集的 CPU 表格分类任务。指标是 `neg_mean_balanced_accuracy`。
- **`tasks/hard-interactions/`**：合成表格分类任务，刻意构造为困难、具有大提升空间，用于给框架超参数实验真正的区分力。指标是 `neg_mean_test_accuracy`。

## 3. 快速开始

### 3.1 开发

要继续开发框架，在此目录打开常规 opencode 会话并说：
...
要启动完整的自主实验，以实验智能体为主线程启动专用 opencode 会话：
...
**实验协议（编码在 `autoresearch-experiment` 智能体定义中）**：主会话执行的最高优先级实验协议。定义如何初始化运行、创建候选方案、调用智能体、调优超参数、解析结果并决定保留/丢弃。

**`AGENTS.md`**：opencode 的项目入口点。新会话首先读取此文件，然后是 `autoresearch-experiment` 智能体定义（编码实验协议），再然后是特定任务的 `TASK.md` 和 `task.toml`。
...
使用 `opencode --agent autoresearch-experiment` 时，智能体在内部编码此协议，不依赖任何外部协议文件。它必须作为主线程启动，以便能为 idea-generator、candidate-writer、tunable-contract-extractor、tuner-orchestrator 生成独立的 `Task(...)` 上下文（崩溃诊断使用 `crash-diagnosis` skill 内联）。
...
**重要约束**：opencode 子智能体缺少 `Agent` 工具（无法生成其他智能体）和 `Skill` 工具（必须直接 Read SKILL.md）。因此，智能体有两种使用模式：

- **运行实验协议的主会话**：可以直接生成 `background-researcher`、`idea-generator`、`experience-extractor`、`candidate-writer`、`tunable-contract-extractor`、`tuner-orchestrator`，并在主上下文中内联执行 `crash-diagnosis` skill。
- **通过 `opencode --agent autoresearch-experiment` 的专用实验会话**：`autoresearch-experiment` 成为主线程，可以继续生成其他六个智能体。
...
职责：
- 完全自包含地执行完整实验协议，不依赖任何外部协议文件
- 初始化新的 `runs/<task>/<tag>/`（设置 = 仅 `background-researcher`；无种子阶段——循环通过 `fresh` 候选方案自举）
- 按**轮次**推进循环：一代 ≤B 个想法经过 step 0+1，然后一次解耦深度调优步骤
- 生成 `Task(idea-generator)` 用于 SELECT + IDEATE
- 生成 `Task(candidate-writer)` 编写候选代码
- 生成 `Task(tunable-contract-extractor)` 提取调优器合约并执行 step 0+1
- 生成 `Task(tuner-orchestrator)` 用于每轮一次的解耦深度调优
...
`ledger.json` 的唯一写入者，以及实验状态的结构化真相源（每个候选一个 JSON 记录，合并原始 `idea_log.md` 想法字段与 `results.tsv` 分数/状态）。**永不手动编辑**；仅通过子命令变更，确保长时间运行中的模式稳定性（见 `.opencode/rules/ledger.md`）：
...
CPU 表格分类模型搜索任务。指标是 `neg_mean_test_accuracy`（越小越好；普通准确率取负；框架在所有地方最小化）。
...
CPU 表格分类任务，跨**三个真实数据集**，以**匿名化**形式提供（`d0`/`d1`/`d2`）。指标是 `neg_mean_balanced_accuracy`（越小越好；平衡准确率取负；框架在所有地方最小化）。数据集的标识、特征含义以及哪种模型/超参数占优均被刻意隐瞒——**核心是：通过实验发现**，而非查找。数据集选择使得仔细调优能带来大幅提升，且需要漫长的搜索才能达到——奖励良好的搜索/调优。

### 8.4 hard-interactions

```text
tasks/hard-interactions/
```

CPU 友好的合成表格分类任务，**刻意构造为困难、具有大提升空间**——以使更好的搜索和更深的调优能产生明显更好的分数。用于给框架超参数实验真正的区分力（不像噪声上限任务那样会立即平台化）。指标是 `neg_mean_test_accuracy`（越小越好；普通准确率取负；框架在所有地方最小化）。目标依赖埋在众多纯噪声特征中的**高阶特征交互**（XOR/奇偶校验、乘积符号、阈值合取）；只有捕获交互的候选方案（足够深度的 GBDT、显式多项式/交互特征、MLP、调优良好的）才能接近 ~88–92% 的上限，而线性/浅模型停在 ~60%。
...
1. **`README.md`**：理解整体结构
2. **`AGENTS.md`**：理解 opencode 如何进入项目
3. **`autoresearch-experiment` 智能体定义**：理解完整实验协议
4. **`.opencode/agents/*.md`**：理解每个子智能体的责任边界
5. **`.opencode/skills/*/SKILL.md`**：理解可复用的方法论
