...
defaults. Every search command also has a deterministic wall-clock batch guard
derived from `run.timeout_seconds` (or `tuner.search_wall_seconds`). It
checkpoints each attempted trial and may return `status: partial`; re-run the
same command to resume toward the same total target.
...
4. Parse the stdout JSON and branch on `status`:
   - `partial` — the safe wall-clock batch ended before the durable total trial
     target. **Re-run the exact same search command.** It resumes from
     `tune_report.json`, does not restart the recorded trial count or deferred
     config queue, and treats
     `--n-trials` / `--max-evals` as a total target rather than "that many more".
     Repeat until it returns `ok`, `failed`, or `rejected`. Do not proceed to
     Apply while a stage is partial.
...
- **No uncapped probes.** Never call `prepare.evaluate_config` from an ad-hoc
  Python snippet. If diagnosis needs one explicit config, use
  `tools/tuners/eval_config.py`; it inherits `per_runtime_limit` and cleans up
  the evaluator process group on interruption.
