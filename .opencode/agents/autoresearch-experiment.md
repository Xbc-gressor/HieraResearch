...
When an agent runs as the main thread, opencode can provide the `Task`
...
Code subagents cannot spawn other subagents; in that mode the `Task` tool is
...
2. `README_ZH.md` if present, for project context.
...
Do not require any external protocol file.
...
Timeouts and failed score-function calls count as attempted evaluations in
`trials_completed`; they consumed the same wall budget even though they produced
no selectable score. Search scripts enforce their own shorter wall-clock batches
and checkpoint after every attempt, so a tool/session interruption is resumed
instead of restarting a fresh trial budget.
