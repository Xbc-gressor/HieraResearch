...
- **exit 3 (CRASHED)** — the config at `crash_index` raised; its **full
  ...
  A `failure_status: timeout` is a real runtime failure, not a successful
  `Infinity` score: it is never cached and must follow the same diagnose/fix or
  abandon path.
...
If you need to isolate one config during diagnosis, run the capped helper:

```bash
uv --directory <env.project> run python tools/tuners/eval_config.py \
  --candidate-path <train_py> --params-json <one-config.json>
```

Never call `prepare.evaluate_config` directly or launch an uncapped ad-hoc
Python probe. A diagnostic inherits the same `per_runtime_limit` as warm-start;
after two diagnostic probes without a concrete fix, abandon the candidate rather
than spending the outer command budget on speculation.
