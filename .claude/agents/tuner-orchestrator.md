...
3. Run the corresponding script (it reads the step-1 **evaluated** warm trials from
   ...
   **Never wrap this call in an external timeout.** No `timeout N ...`, no
   ...
   foreground calls hitting the Bash tool's 2-minute default. Use
   `run_in_background: true` and poll, and let the script's own
   `per_runtime_limit` bound each single evaluation — that is the only
   duration guard that exists, and it is already correct.
