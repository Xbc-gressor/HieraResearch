...
description: |
  Incrementally revise the ledger's compact global experience from new or changed
  DAG edges, bounded Top/Bottom anchors, and validated `tf-*` lineage. Emit evidence-bound levers and run-local
  direction status without changing records or external credibility. Return a
  receipt; the experience block is the payload.
