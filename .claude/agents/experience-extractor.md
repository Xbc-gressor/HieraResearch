...
description: |
  Incrementally regenerate the bounded run-level belief snapshot from DAG
  deltas, fixed anchors, and mechanically rendered semantic mappings. Keeps
  observations, attribution, and derived belief separate, emits only the
  documented snapshot schema, and requests deterministic search-space state
  transitions.
