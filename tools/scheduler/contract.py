...
class ResourceContract:
    ...
    def __post_init__(self) -> None:
        for name in ("bout_trials", "max_bouts", "k_eval"):
            value = getattr(self, name)
            if not isinstance(value, int) or isinstance(value, bool) or value <= 0:
                raise ValueError(f"{name} must be a positive integer")

    def bout_admissible(self, remaining_budget: int) -> bool:
        """Whether the remaining global budget funds one complete bout."""
        return remaining_budget >= self.bout_trials
...
def ineligibility_reason(
    ...
    if not contract.bout_admissible(remaining_budget):
        return (
            f"remaining budget {remaining_budget} cannot admit a full "
            f"{contract.bout_trials}-trial bout"
        )
    return None
