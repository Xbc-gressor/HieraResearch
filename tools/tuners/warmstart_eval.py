...
def main() -> int:
    ...
    previous_phase_a = read_tune_report(args.tune_report_json).get("phase_a", {})
    prev = previous_phase_a.get("warm_start_configs", [])
    previous_elapsed = float(previous_phase_a.get("elapsed_seconds", 0.0) or 0.0)
    cache = {_params_key(t["params"]): t["score"] for t in prev
             if isinstance(t.get("score"), (int, float))
             and math.isfinite(float(t["score"]))}
    ...
    for i, raw in enumerate(configs):
        ...
        if key in cache:
            ...
            try:
                ...
                sys.stderr.write(tb)
                failure_status = exc.status if isinstance(exc, EvaluationFailure) else "error"
                wsc.append({"params": params, "score": None, "status": failure_status,
                            "error": f"{type(exc).__name__}: {exc}"[:300],
                            "error_traceback": tb})
                report["phase_a"]["warm_start_configs"] = wsc
                report["phase_a"]["status"] = "crashed"
                report["phase_a"]["elapsed_seconds"] = round(
                    previous_elapsed + time.time() - started, 1
                )
                write_tune_report(args.tune_report_json, report)
