...
def _iter_trials(report: dict):
    ...
    for warm in phase_a.get("warm_start_configs", []):
        if (isinstance(warm.get("score"), (int, float))
                and math.isfinite(float(warm["score"]))):
...
def _attempt_count(report: dict) -> int:
    """Count score-function calls, including timeouts and failed trials."""
    phase_a = report.get("phase_a", {})
    total = len(phase_a.get("warm_start_configs", []))
    for stage in report.get("phase_c", {}).get("stages", []):
        total += len(stage.get("trials", []))
    return total
...
def summarize(report: dict) -> dict:
    ...
    return {
        ...
        "trials_completed": _attempt_count(report),
