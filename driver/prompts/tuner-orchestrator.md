...
- **`run_id` is `null`** → no candidate is eligible this round (below
  ...
  names the candidate *selected for a bout*, and none was. Each settled
  candidate's trials and terminal status are recorded durably by
  `finalize_tuning.py` in its report and the ledger. If nothing is stranded,
  submit a receipt with `tuned_run_id: "none"`, `tuned: false`, and
  `ledger_updated: false`, then **stop**. This is a valid no-op.
...
4. Wait for the detached search to exit, then parse the terminal JSON **from
   ...
   **not** interrupted, however long it takes: do not relaunch it, do not run
   ...
   - **no terminal JSON object** (crash, kill, or truncated log) — the
     ...
     interrupts, submit a receipt with `tuned_run_id: "none"`, `tuned: false`,
     and `ledger_updated: false`. The candidate stays untuned
   ...
   - **one terminal JSON object** — run `phase-c-action` again against the
     ...
     `budget_exhausted` close still keeps its scored trials eligible. Never
     hand-construct a
...
edits. Submit a receipt with `tuned_run_id: "none"`, `tuned: false`, and
`ledger_updated: false`. A partial report must leave the candidate untuned.
...
that candidate; the close and finalization record the settled trials durably.

## Reconciliation duty

When your invocation context carries `reconcile_note`, the driver is telling
you a previous tuner session's receipt disagrees with the on-disk state (or
the session failed mid-work). Reconcile from the AUTHORITATIVE artifacts —
the ledger, `tune_report.json`, the attempt log, and finalization artifacts —
never from the prior receipt. Complete any valid pending finalization ONLY via
`tools/finalize_tuning.py`, or submit a corrected truthful no-op receipt
(`tuned_run_id: "none"`, `tuned: false`, `ledger_updated: false`). Never
hand-edit the report or the ledger to make the story agree.
...
You are running as one invocation of the `tuner-orchestrator` role, spawned by the
...
- `tuned_run_id` — str — the run id of the candidate selected for a bout this
  invocation, or `"none"` for a valid no-op (including a stranded-work-only
  sweep).
- `tuned` — bool — whether a bout was finalized this invocation
