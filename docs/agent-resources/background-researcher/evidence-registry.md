...
structured guidance · hypothesis preservation.
...
full containment is partial. Only direct guidance may alter hypothesis
eligibility. This deliberately makes a false broad claim fail open (the
...
only produce `caution`; it cannot deprioritize or exclude. A binding negative
...
tests. Every external `deprioritize`/`exclude` guidance item needs at least
...
it a stable `hyp-*` identity and let the typed matcher derive `active`,
